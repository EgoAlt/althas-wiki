...
function buildMarkerElement(
    ...
    // A pin whose target page declares a `submap:` gets a drill-down popup
    // client-side (page link + "Open local map") instead of navigating on
    // the first click. Local submap pins never set this: their link is the
    // page they already live on.
    const hasSubmap =
        !marker.local && LEAFLET_MAP_PLUGIN_DATA.submapSlugs.has(marker.link as string);
    return {
        type: "element",
        tagName: "div",
        properties: {
